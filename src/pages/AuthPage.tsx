...
export default function AuthPage({ onNavigate }: AuthPageProps) {
    ...
    try {
      if (mode === 'login') {
        ...
        if (error) {
          ...
          setInfo('Si cet email existe, vous recevrez un lien de reinitialisation.');
        }
      }
    } finally {
      setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] flex flex-col items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[500px] bg-[#39FF14]/4 rounded-full blur-[160px]" />

      <div className="relative z-10 w-full max-w-md">
        <button onClick={() => onNavigate('landing')} className="flex items-center gap-2 text-sm text-gray-500 hover:text-[#39FF14] transition-colors mb-8">
        ...
        <div className="flex items-center gap-2 mb-8">
          ...
          <span className="text-xl font-black">Dziko<span className="text-[#39FF14]">Vinted</span></span>
        </div>
        ...
        <div className="bg-[#181818] border border-white/8 rounded-2xl p-8">
          ...
          {info && (
            <div className="flex items-center gap-3 bg-[#39FF14]/10 border border-[#39FF14]/20 rounded-xl px-4 py-3 mb-6">
              <AlertCircle className="w-4 h-4 text-[#39FF14] flex-shrink-0" />
              <p className="text-sm text-[#39FF14]">{info}</p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-[#0A0A0A] border border-white/10 rounded-xl pl-10 pr-4 py-3 text-sm text-gray-200 placeholder:text-gray-600 focus:outline-none focus:border-[#39FF14]/40 focus:ring-1 focus:ring-[#39FF14]/20 transition-all"
                  />
                </div>
              </div>
            )}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full bg-[#0A0A0A] border border-white/10 rounded-xl pl-10 pr-10 py-3 text-sm text-gray-200 placeholder:text-gray-600 focus:outline-none focus:border-[#39FF14]/40 focus:ring-1 focus:ring-[#39FF14]/20 transition-all"
                  />
            ...
            {mode === 'login' && (
              <div className="flex justify-end">
                <button type="button" onClick={() => { setMode('forgot'); setError(null); }} className="text-xs text-gray-500 hover:text-[#39FF14] transition-colors">
                  Mot de passe oublié ?
                </button>
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#39FF14] text-black font-bold py-3.5 rounded-xl hover:bg-[#50ff30] transition-all duration-200 hover:shadow-[0_0_30px_rgba(57,255,20,0.3)] disabled:opacity-50 disabled:cursor-not-allowed text-sm mt-2"
            >
          ...
          <div className="border-t border-white/5 mt-6 pt-6 text-center">
            {mode === 'login' ? (
              <p className="text-sm text-gray-500">Pas encore de compte ?{' '}
                <button onClick={() => { setMode('register'); setError(null); }} className="text-[#39FF14] hover:underline font-medium">Créer un compte</button>
              </p>
            ) : (
              <p className="text-sm text-gray-500">Déjà un compte ?{' '}
                <button onClick={() => { setMode('login'); setError(null); }} className="text-[#39FF14] hover:underline font-medium">Se connecter</button>
              </p>
            )}
          </div>
        </div>
