  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-400 text-sm">Tu es sur le plan <span className={`font-bold ${currentPlan === 'pro' || currentPlan === 'team' ? 'text-[#39FF14]' : 'text-gray-300'}`}>{currentPlan.toUpperCase()}</span>.</p>
      </div>

      {/* Current plan banner */}
      <div className="bg-[#181818] border border-[#39FF14]/20 rounded-2xl p-5 mb-8 flex flex-col sm:flex-row items-start sm:items-center gap-4">
        <div className="w-10 h-10 bg-[#39FF14]/10 rounded-xl flex items-center justify-center flex-shrink-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {plans.map((plan) => {
          ...
          return (
            <div key={plan.name} className={`relative bg-[#181818] border rounded-2xl p-7 flex flex-col transition-all duration-300 hover:-translate-y-1 ${plan.highlighted ? 'border-[#39FF14]/30 shadow-[0_0_50px_rgba(57,255,20,0.08)] md:scale-105' : isCurrent ? 'border-[#39FF14]/20' : 'border-white/5'}`}>
              {plan.highlighted && !isCurrent && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#39FF14] text-black text-xs font-bold px-4 py-1 rounded-full">Le plus populaire</div>
              )}
              {isCurrent && (
              ...
              <div className="mb-5">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${plan.highlighted ? 'bg-[#39FF14]/15' : 'bg-white/5'}`}>
                  <Icon className={`w-5 h-5 ${plan.highlighted ? 'text-[#39FF14]' : 'text-gray-400'}`} />
                </div>
              ...
              <button
                disabled={isCurrent}
                className={`w-full py-3 rounded-xl font-semibold text-sm transition-all duration-200 ${isCurrent ? 'bg-white/5 text-gray-500 cursor-not-allowed' : plan.highlighted ? 'bg-[#39FF14] text-black hover:bg-[#50ff30] hover:shadow-[0_0_30px_rgba(57,255,20,0.3)] active:scale-[0.98]' : 'bg-white/5 text-gray-300 hover:bg-white/10 active:scale-[0.98]'}`}
              >
