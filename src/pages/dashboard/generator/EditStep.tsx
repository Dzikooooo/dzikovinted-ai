          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {editForm.keywords.map((kw, i) => (
              <span key={i} className="flex items-center gap-1.5 px-3 py-1 text-xs font-mono bg-neon-500/10 text-neon-500 rounded-full border border-neon-500/20">
                #{kw.replace(/\s+/g, '')}
                <button onClick={() => updateField('keywords', editForm.keywords.filter((_, j) => j !== i))} aria-label={`Supprimer le mot-clé ${kw}`} className="text-neon-500/40 hover:text-red-400 transition-colors ml-1">
                  <X className="w-3 h-3" />
                </button>
              </span>
            ))}
          </div>
          <input type="text" placeholder="Ajouter un mot-clé (Entrée)" className="w-full bg-dark-400 border border-white/10 rounded-xl px-4 py-3 text-sm text-gray-200 focus:outline-none focus:border-neon-500/40 focus:ring-2 focus:ring-neon-500/20 transition-all"
