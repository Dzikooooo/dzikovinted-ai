...
function CopyBtn({ text, small }: { text: string; small?: boolean }) {
  ...
  return (
    ...
    >
      {copied
        ? <Check className={small ? 'w-3 h-3 text-[#FFC400]' : 'w-3.5 h-3.5 text-[#FFC400]'} />
...
function FieldCard({ label, value, icon: Icon }: { label: string; value: string; icon: React.ElementType }) {
  return (
    <div className="bg-[#0A0A0A] border border-white/5 rounded-xl p-4 hover:border-[#FFC400]/20 transition-colors">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <Icon className="w-3.5 h-3.5 text-[#FFC400]/60" />
          <span className="text-[10px] font-mono uppercase tracking-wider text-[#FFC400]/60">{label}</span>
        </div>
      ...
      <div className="p-4 sm:p-6 lg:p-8 max-w-3xl mx-auto">
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl sm:text-3xl font-black mb-2">Generateur <span className="text-[#FFC400]">IA</span></h1>
            ...
            {limit !== null && (
              <div className="hidden sm:flex items-center gap-2 bg-[#181818] border border-white/5 rounded-xl px-4 py-2.5">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-[#FFC400]">{credits} <span className="text-gray-600 font-normal">/ {limit}</span></p>
                </div>
              </div>
            )}
          </div>
          ...
          {limit !== null && (
            <div className="sm:hidden flex items-center gap-2 bg-[#181818] border border-white/5 rounded-xl px-4 py-2.5 mt-4">
              <Zap className="w-4 h-4 text-[#FFC400]" />
              <p className="text-sm"><span className="font-bold text-[#FFC400]">{credits}</span> <span className="text-gray-500">credits restants sur {limit}</span></p>
            </div>
          )}
        </div>
        ...
        {isLimitReached && (
          <div className="bg-[#181818] border border-orange-400/20 rounded-2xl p-6 mb-6 text-center">
            ...
            <p className="text-xs text-gray-400">Passe au plan <span className="text-[#FFC400] font-bold">Pro</span> pour des analyses illimitees.</p>
          </div>
        )}

        <div
          className={`bg-[#181818] border rounded-2xl transition-all duration-300 ${isDragging ? 'border-[#FFC400]/50 shadow-[0_0_30px_rgba(57,255,20,0.15)]' : 'border-white/5'} ${isLimitReached ? 'opacity-50 pointer-events-none' : ''}`}
        ...
        >
          {images.length === 0 ? (
            <label className="flex flex-col items-center justify-center py-20 sm:py-24 cursor-pointer group">
              <div className="w-20 h-20 rounded-2xl bg-[#0A0A0A] border-2 border-dashed border-gray-700 flex items-center justify-center mb-6 group-hover:border-[#FFC400]/40 group-hover:bg-[#FFC400]/5 transition-all duration-300">
                <Upload className="w-8 h-8 text-gray-600 group-hover:text-[#FFC400] transition-colors" />
              </div>
            ...
            <div className="p-5 sm:p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                {images.map((src, i) => (
                  <div
                    ...
                    className={`relative group aspect-square rounded-xl overflow-hidden bg-[#0A0A0A] border transition-all cursor-grab active:cursor-grabbing ${
                      dragOverIdx === i ? 'border-[#FFC400] shadow-[0_0_15px_rgba(57,255,20,0.2)]' : i === 0 ? 'border-[#FFC400]/40 ring-1 ring-[#FFC400]/20' : 'border-white/5'
                  ...
                  >
                    ...
                    <div className="absolute bottom-2 right-2 flex flex-col gap-1 sm:hidden">
                      {i > 0 && (
                        <button
                          ...
                          className="w-7 h-7 rounded-md bg-black/80 border border-white/10 flex items-center justify-center active:bg-[#FFC400]/20"
                        >
                    ...
                    <div className="absolute bottom-2 left-2 sm:opacity-0 sm:group-hover:opacity-100 transition-all">
                      <span className={`text-[10px] font-mono px-2 py-0.5 rounded-md border ${
                        i === 0 ? 'bg-[#FFC400]/20 text-[#FFC400] border-[#FFC400]/30' : 'bg-black/60 text-white border-white/10'
                      }`}>
                ...
                {images.length < 4 && (
                  <label className="aspect-square rounded-xl border-2 border-dashed border-gray-700 flex flex-col items-center justify-center cursor-pointer hover:border-[#FFC400]/40 hover:bg-[#FFC400]/5 transition-all group">
                    <ImageIcon className="w-5 h-5 text-gray-600 group-hover:text-[#FFC400] mb-1 transition-colors" />
                    <span className="text-xs text-gray-600 group-hover:text-[#FFC400] transition-colors">Ajouter</span>
                    <input type="file" accept="image/*" multiple className="hidden" onChange={(e) => handleFiles(e.target.files)} />
                  </label>
                )}
              </div>
              <div className="mb-5 bg-[#0A0A0A] border border-white/5 rounded-xl p-4">
      ...
      <button
        ...
        className={`rounded-xl px-3 py-2 text-xs font-bold border transition ${
          photoStyle === style.id
            ...
            : "bg-[#181818] text-gray-400 border-white/10"
        }`}
      >
        {style.label}
      </button>
...
<div className="mb-5 bg-[#0A0A0A] border border-white/5 rounded-xl p-4">
        ...
        {item.label}
              ...
              <div className="flex items-center justify-between pt-2 border-t border-white/5">
                ...
                <button
                  ...
                  className="flex items-center gap-2 bg-[#FFC400] text-black font-bold px-6 py-2.5 rounded-xl hover:bg-[#D89B00] transition-all hover:shadow-[0_0_20px_rgba(57,255,20,0.3)] text-sm disabled:opacity-40 disabled:hover:shadow-none"
                >
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={s} className="flex items-start gap-3 bg-[#181818]/50 border border-white/3 rounded-xl px-4 py-3">
              <span className="text-[10px] font-mono text-[#FFC400] bg-[#FFC400]/10 px-2 py-0.5 rounded-md flex-shrink-0 mt-0.5">{s}</span>
              <div>
      ...
      <div className="flex items-center justify-center min-h-[calc(100vh-4rem)] p-8">
        <div className="text-center max-w-sm w-full">
          <div className="relative w-28 h-28 mx-auto mb-10">
            <div className="absolute inset-0 rounded-full border-2 border-[#FFC400]/20 animate-spin" style={{ animationDuration: '3s' }} />
            <div className="absolute inset-3 rounded-full border-2 border-dashed border-[#FFC400]/40 animate-spin" style={{ animationDirection: 'reverse', animationDuration: '2s' }} />
            <div className="absolute inset-0 flex items-center justify-center">
              <Sparkles className="w-10 h-10 text-[#FFC400] animate-pulse" />
            </div>
          </div>
          ...
          <div className="space-y-3">
            {LOADING_MESSAGES.map(({ text, sub }, i) => {
              ...
              return (
                <div key={text} className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all duration-500 ${isActive ? 'bg-[#FFC400]/10 border-[#FFC400]/30 shadow-[0_0_20px_rgba(57,255,20,0.1)]' : isDone ? 'bg-[#FFC400]/5 border-[#FFC400]/10' : 'bg-[#181818] border-white/5'}`}>
                  <div className={`w-2 h-2 rounded-full flex-shrink-0 ${isActive ? 'bg-[#FFC400] animate-pulse' : isDone ? 'bg-[#FFC400]/60' : 'bg-gray-700'}`} />
                  <div className="flex-1 text-left">
                    <span className={`text-sm block ${isActive ? 'text-[#FFC400] font-medium' : isDone ? 'text-gray-400' : 'text-gray-600'}`}>{text}</span>
                    {isActive && <span className="text-[10px] text-[#FFC400]/50 block mt-0.5">{sub}</span>}
                  </div>
                  {isDone && <Check className="w-4 h-4 text-[#FFC400]/70 ml-auto" />}
                  {isActive && (
                    <div className="ml-auto flex gap-1">
                      {[0, 200, 400].map((d) => <div key={d} className="w-1 h-1 rounded-full bg-[#FFC400] animate-bounce" style={{ animationDelay: `${d}ms` }} />)}
                    </div>
                  )}
                </div>
              );
  ...
  if (step === 'result' && result) {
    return (
      <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
        <button onClick={resetAll} className="flex items-center gap-2 text-sm text-gray-400 hover:text-[#FFC400] transition-colors mb-6">
          <ArrowLeft className="w-4 h-4" /> Nouvelle annonce
        </button>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          <div>
  <h1 className="text-2xl sm:text-3xl font-black mb-1">
    Annonce <span className="text-[#FFC400]">prête</span>
  </h1>
  <p className="text-gray-400 text-sm">Optimisée pour Vinted</p>
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <button onClick={() => setStep('edit')} className="flex items-center gap-2 border border-[#FFC400]/30 text-[#FFC400] font-medium px-4 py-2 rounded-xl hover:bg-[#FFC400]/10 transition-all text-sm">
              <Pencil className="w-4 h-4" /> Modifier
            </button>
            <button
              onClick={handleSave}
              disabled={saving || saved}
              className={`flex items-center gap-2 font-medium px-4 py-2 rounded-xl transition-all text-sm ${saved ? 'bg-[#FFC400]/20 text-[#FFC400] border border-[#FFC400]/30' : 'bg-[#FFC400] text-black hover:bg-[#D89B00] hover:shadow-[0_0_20px_rgba(57,255,20,0.3)] disabled:opacity-60'}`}
            >
        ...
        <div className="bg-[#181818] border border-white/5 rounded-2xl p-5 sm:p-7 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { label: 'Prix recommande', val: `${result.price} EUR`, color: 'text-[#FFC400]', border: 'border-[#FFC400]/20', bg: 'bg-[#FFC400]/5' },
          ...
          <div className="bg-[#0A0A0A] border border-white/5 rounded-xl p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Tag className="w-3.5 h-3.5 text-[#FFC400]/60" />
                <span className="text-[10px] font-mono uppercase tracking-wider text-[#FFC400]/60">Hashtags / Mots-cles</span>
              </div>
              <CopyBtn text={result.keywords.map(k => `#${k.replace(/\s+/g, '')}`).join(' ')} />
            </div>
            <div className="flex flex-wrap gap-2">
              {result.keywords.map((kw) => (
                <span key={kw} onClick={() => navigator.clipboard.writeText(`#${kw.replace(/\s+/g, '')}`)} className="px-3 py-1 text-xs font-mono bg-[#FFC400]/10 text-[#FFC400] rounded-full border border-[#FFC400]/20 cursor-pointer hover:bg-[#FFC400]/20 transition-colors">
                  #{kw.replace(/\s+/g, '')}
                </span>
              ))}
            </div>
          ...
          <div className="bg-[#0A0A0A] border border-white/5 rounded-xl p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Filter className="w-3.5 h-3.5 text-[#FFC400]/60" />
                <span className="text-[10px] font-mono uppercase tracking-wider text-[#FFC400]/60">Filtres Vinted</span>
              </div>
              <CopyBtn text={result.vinted_filters.map((f) => `${f.label}: ${f.value}`).join('\n')} />
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {result.vinted_filters.map((f) => (
                <div key={f.label} onClick={() => navigator.clipboard.writeText(f.value)} className="bg-[#181818] rounded-lg px-3 py-2 border border-white/5 cursor-pointer hover:border-[#FFC400]/20 transition-colors">
  ...
  if (step === 'edit' && editForm) {
    return (
      <div className="p-4 sm:p-6 lg:p-8 max-w-3xl mx-auto">
        <button onClick={() => setStep('result')} className="flex items-center gap-2 text-sm text-gray-400 hover:text-[#FFC400] transition-colors mb-6">
          <ArrowLeft className="w-4 h-4" /> Retour au resultat
        </button>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          <h1 className="text-2xl sm:text-3xl font-black">Modifier l'<span className="text-[#FFC400]">annonce</span></h1>
          <div className="flex gap-2">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-[#FFC400] text-black font-bold px-4 py-2 rounded-xl hover:bg-[#D89B00] transition-all text-sm"
            >
        ...
        <div className="bg-[#181818] border border-white/5 rounded-2xl p-5 sm:p-7 space-y-5">
          <div>
            <label className="text-[10px] font-mono uppercase tracking-wider text-[#FFC400]/60 block mb-2">Titre SEO</label>
            <input type="text" className="w-full bg-[#0A0A0A] border border-white/10 rounded-xl px-4 py-3 text-sm text-gray-200 focus:outline-none focus:border-[#FFC400]/40 transition-all" value={editForm.title} onChange={(e) => updateEdit('title', e.target.value)} />
          </div>
          <div>
            <label className="text-[10px] font-mono uppercase tracking-wider text-[#FFC400]/60 block mb-2">Description</label>
            <textarea className="w-full bg-[#0A0A0A] border border-white/10 rounded-xl px-4 py-3 text-sm text-gray-200 focus:outline-none focus:border-[#FFC400]/40 transition-all min-h-[120px] resize-y" value={editForm.description} onChange={(e) => updateEdit('description', e.target.value)} />
          </div>
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <div key={k}>
                ...
                <input type="number" className="w-full bg-[#0A0A0A] border border-white/10 rounded-xl px-4 py-3 text-sm text-gray-200 focus:outline-none focus:border-[#FFC400]/40 transition-all" value={(editForm as unknown as Record<string, number>)[k]} onChange={(e) => updateEdit(k as keyof GeneratedListing, parseFloat(e.target.value) || 0)} />
              </div>
            ))}
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {[
              ...
              <div key={k}>
                <label className="text-[10px] font-mono uppercase tracking-wider text-gray-500 block mb-2">{label}</label>
                <input type="text" className="w-full bg-[#0A0A0A] border border-white/10 rounded-xl px-4 py-3 text-sm text-gray-200 focus:outline-none focus:border-[#FFC400]/40 transition-all" value={(editForm as unknown as Record<string, string>)[k]} onChange={(e) => updateEdit(k as keyof GeneratedListing, e.target.value)} />
              </div>
            ))}
            <div>
              <label className="text-[10px] font-mono uppercase tracking-wider text-gray-500 block mb-2">Etat</label>
              <select className="w-full bg-[#0A0A0A] border border-white/10 rounded-xl px-4 py-3 text-sm text-gray-200 focus:outline-none focus:border-[#FFC400]/40 transition-all" value={editForm.condition} onChange={(e) => updateEdit('condition', e.target.value)}>
          ...
          <div>
            <label className="text-[10px] font-mono uppercase tracking-wider text-[#FFC400]/60 block mb-2">Mots-cles / Hashtags</label>
            <div className="flex flex-wrap gap-2 mb-2">
              {editForm.keywords.map((kw, i) => (
                <span key={i} className="flex items-center gap-1.5 px-3 py-1 text-xs font-mono bg-[#FFC400]/10 text-[#FFC400] rounded-full border border-[#FFC400]/20">
                  #{kw.replace(/\s+/g, '')}
                  <button onClick={() => updateEdit('keywords', editForm.keywords.filter((_, j) => j !== i))} className="text-[#FFC400]/40 hover:text-red-400 transition-colors ml-1">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
            </div>
            <input type="text" placeholder="Ajouter un mot-cle (Entree)" className="w-full bg-[#0A0A0A] border border-white/10 rounded-xl px-4 py-3 text-sm text-gray-200 focus:outline-none focus:border-[#FFC400]/40 transition-all"
