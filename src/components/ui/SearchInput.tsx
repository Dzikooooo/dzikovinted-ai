...
export function SearchInput({ className = '', ...rest }: SearchInputProps) {
  return (
    <div className={`relative ${className}`}>
      ...
      <input
        type="text"
        className="w-full bg-surface border border-gray-200 rounded-xl pl-10 pr-4 py-2.5 text-sm text-gray-700 placeholder:text-gray-500 focus:outline-none focus:border-neon-500/30 focus:ring-2 focus:ring-neon-500/20"
        {...rest}
      />
    </div>
  );
