...
- **Watchlist personnelle** — ✅ fait (2026-07-12) : `watchlist` passe de liste globale unique à personnelle par utilisateur (`src/pages/dashboard/WatchlistPage.tsx`, CRUD complet), les 7 recherches de départ restant des recherches plateforme en lecture seule. Le scan dédoublonne les paires marque/modèle identiques entre utilisateurs avant de scanner (`scripts/watchlistDedup.ts`) pour ne jamais payer deux fois le même coût de scan — voir `ARCHITECTURE.md` §4.9. Deux champs déjà calculés par le moteur d'opportunités mais jamais affichés (`competing_listings_count`, `first_observed_at`) sont désormais visibles sur chaque carte d'opportunité.
