...
- **Phase B (interface multi-comptes)** : ✅ codée — sélecteur de compte premium dans la sidebar (`AccountSwitcher`, panneau `.glass-card`, recherche au-delà de 6 comptes), gestion complète dans Paramètres > Comptes Vinted (renommage inline, définir par défaut via la RPC `set_default_vinted_account`, suppression avec confirmation), avatars à initiales colorées déterministes (`AccountAvatar`), état de filtre partagé (`VintedAccountFilterContext`, persisté en `localStorage`). Dashboard et page Compte Vinted fonctionnent dans les deux modes (vue globale agrégée vs vue filtrée par compte) partout où les données sont déjà scopées par compte (`vinted_accounts`/`vinted_listings`). Stock/Comptabilité/Statistiques restent volontairement non filtrés — leurs données ne sont pas encore rattachées à un compte (Phase C), pas de faux filtre inactif. Vérifié par `typecheck`/`lint`/`build` ; la vérification visuelle live a été bloquée par une panne de connexion de l'outil de navigateur pendant la session — à confirmer visuellement à la prochaine session avant la Phase C.
...
- **Limite de test connue** : un seul compte Vinted réel disponible pour les tests (`alexisdzk`) — valider un vrai deuxième compte (bascule, recherche, gestion multi-comptes) nécessitera une connexion à un second compte Vinted dans le navigateur de test.
