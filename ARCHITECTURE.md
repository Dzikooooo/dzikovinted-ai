...
listings (status: draft → en_stock → vendu)
  → StockPage (onglet "ResellOS") : marque un item comme vendu (sold_price, fees, sold_date)
...
**`listings` et `vinted_listings` restent deux modèles séparés, sans lien entre eux (2026-07-09)** : `StockPage.tsx` a un second onglet "Vinted" (par défaut), qui affiche le miroir réel de `vinted_listings` filtré par le compte Vinted actif (`VintedAccountFilterContext`), avec sa propre logique de statut (`online`/`reserved`/`sold_pending`/`sold_completed`/...) et son propre calcul — CA confirmé (`sold_completed`) et CA en attente (`sold_pending`), pas de bénéfice/ROI puisque `vinted_listings` n'a pas de prix d'achat. Un vrai rapprochement des deux tables (pour calculer un bénéfice réel par compte Vinted) reste à faire — voir [EXTENSION.md](EXTENSION.md) §5.
...
- **Stock (onglet ResellOS)** : suivi des annonces (brouillon/en stock/vendu), marquage de vente avec frais, calcul de marge/ROI par article
- **Stock (onglet Vinted)** : miroir réel du compte Vinted actif (ou cumulé en vue "Tous les comptes"), statuts en ligne/réservé/vente en cours/vendu/brouillon/masqué, synchronisation à la demande — voir [EXTENSION.md](EXTENSION.md) §5
