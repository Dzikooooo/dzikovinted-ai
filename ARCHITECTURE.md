...
**Messages d'erreur (Cycle B pré-bêta, 2026-07-24)** : `src/lib/errorMessages.ts` centralise la traduction des erreurs techniques en français compréhensible — `translateAuthError()` (signUp/signIn/resetPassword/changePassword), `translateExtensionError()` (appairage/RUN_ACTION), `translateGeneratorError()` (analyse IA). Règle appliquée partout : une cause reconnue devient une phrase claire avec une action concrète ; une cause non reconnue garde son détail technique d'origine entre parenthèses plutôt que d'être remplacée par un message générique inventé — jamais de masquage silencieux de la cause réelle.

**Parcours "Mot de passe oublié" (Cycle C pré-bêta, 2026-07-24)** : jusqu'ici, cliquer le lien reçu par email établissait une session (Supabase la détecte automatiquement dans l'URL) et l'effet de redirection existant de `App.tsx` envoyait aussitôt l'utilisateur au dashboard, sans jamais lui demander de définir un nouveau mot de passe. Corrigé via l'événement officiel Supabase `PASSWORD_RECOVERY` : `AuthContext` expose désormais `passwordRecovery: boolean` (vrai uniquement entre ce clic et la confirmation du nouveau mot de passe) et `clearPasswordRecovery()`. `App.tsx` route en priorité vers le nouvel écran `ResetPasswordPage` (`AppPage = "reset-password"`) tant que `passwordRecovery` est vrai, avant même la redirection automatique vers le dashboard. Une fois le nouveau mot de passe confirmé (`supabase.auth.updateUser`), redirection directe vers le dashboard — la session de récupération est déjà une session authentifiée valide, pas besoin de forcer une reconnexion. `resetPasswordForEmail()` passe désormais explicitement `redirectTo: window.location.origin`, pour ne plus dépendre d'un réglage externe (Site URL du projet Supabase) invisible depuis le code. **Non testé en conditions réelles** (nécessite un clic sur un vrai lien email) — repose sur le pattern officiellement documenté par Supabase, mais le parcours complet (email → clic → écran dédié → dashboard) reste à valider en direct avant la bêta.

**Format monétaire (Cycle D pré-bêta, 2026-07-24)** : `src/lib/currency.ts::formatEUR()` est la seule fonction qui doit produire un montant affiché — "123 €", entier arrondi, espace avant le symbole, jamais le code ISO "EUR". Remplace deux conventions qui coexistaient (Stock/Opportunités en "€" sans espace homogène, Comptabilité/Dashboard/Stats/Dépenses en "EUR" ou 2 décimales). Exception délibérée, pas un oubli : les prix d'abonnement (`Pricing.tsx`, `SubscriptionPage.tsx`) restent affichés avec leurs décimales exactes ("9,99 €") — arrondir un montant réellement facturé serait une fausse information, pas une simple mise en forme.

**Parcours Générateur — corrections Cycle A (audit pré-bêta, 2026-07-24)** : l'audit de bout en bout du parcours Générateur a confirmé cinq problèmes concrets, tous corrigés :
- `GeneratorPage.tsx::handleSave()` faisait toujours un `insert`, jamais un `update` — ré-enregistrer une annonce déjà sauvegardée après modification créait une ligne dupliquée et ré-uploadait les mêmes photos (orphelinant le premier jeu dans Storage). Un nouvel état `savedListingId` bascule sur `update(...).eq('id', ...)` une fois le premier enregistrement réussi ; `durableImageUrls` est capturé au premier upload et réutilisé tel quel sur les sauvegardes suivantes.
- Décision produit validée : le bouton "Sauvegarder" peut rester sur "Sauvegardé !" tant qu'aucun champ n'a changé, mais redevient actif dès qu'un changement réel est fait dans `EditStep` (`handleEditFormChange` réinitialise `saved` à `false` — pas les autres interactions de l'écran, ex. ouvrir "Modifier" seul).
- Décision produit validée : après une sauvegarde réussie, `ResultStep.tsx` affiche une bannière de succès avec deux actions ("Voir dans mon Stock", "Créer une nouvelle annonce") plutôt que de laisser l'utilisateur sans suite — nécessite un `onNavigate` passé depuis `DashboardLayout.tsx` jusqu'à `GeneratorPage`.
- Un crédit est réservé côté serveur dès le lancement de l'analyse (voir `analyze-clothing`) ; quitter l'écran avant la sauvegarde le perdait silencieusement, sans annonce créée ni trace nulle part. Deux garde-fous distincts couvrent les deux façons de quitter : `DashboardLayout.tsx` expose `confirmLeaveGenerator()`/`navigateToPage()` (confirmation avant tout changement de page interne — pas une vraie navigation, juste un changement de state React donc aucun `beforeunload` ne le couvrirait) et `GeneratorPage.tsx` ajoute un écouteur `beforeunload` classique pour le vrai rafraîchissement/fermeture d'onglet. Les deux se basent sur le même signal `busy` (`step === 'loading'` ou résultat généré non encore sauvegardé).
- `translateGeneratorError()` enveloppait par erreur des messages déjà clairs et actionnables renvoyés directement par la fonction Edge (limite de crédits, "Profil introuvable", clé Gemini manquante) dans un message générique — ces trois chaînes reconnues passent maintenant inchangées. `handleSave()` route aussi désormais ses erreurs Postgres/upload à travers ce traducteur au lieu de les afficher brutes.

Non vérifié en conditions réelles dans cette session (pas d'identifiants disponibles dans cet environnement pour se connecter au dashboard) : le rendu effectif de la bannière de succès et du dialogue de confirmation de navigation dans le navigateur. Vérifié : typecheck, lint, build, et les 192 tests (dont 3 nouveaux pour le passthrough de `translateGeneratorError`) passent.

**Parcours Générateur — corrections Cycle B (audit pré-bêta, 2026-07-24)** : trois corrections techniques, sans décision produit à valider :
- `UploadStep.tsx` : la mention statique "1 à 4 photos" du bloc "Comment ça marche" était fausse pour tous les plans (`PLAN_PHOTO_LIMITS` = free:1, pro:10, team:10) — remplacée par `photoLimit`, déjà utilisé dynamiquement partout ailleurs dans ce même fichier.
- `UploadStep.tsx::handleFiles()` rejetait silencieusement les fichiers non-image et les photos dépassant `photoLimit` — aucune indication à l'utilisateur que sa sélection avait été partiellement ignorée. Un état local `warning` calcule désormais les deux cas et affiche un bandeau (même style que le bandeau "Limite atteinte" déjà existant) avec un décompte précis.
- `LoadingStep.tsx` : la progression des 4 étapes affichées est pilotée par un minuteur fixe (`GeneratorPage.tsx::handleGenerate`, 0/800/1600/2400ms), découplé de la durée réelle de l'appel à `analyze-clothing` — au-delà, l'écran restait bloqué sur "Quelques secondes..." sans jamais rassurer l'utilisateur si l'analyse prenait réellement plus longtemps. Un message de repli ("Ça prend plus de temps que d'habitude...") apparaît après 8 secondes.

Le problème des photos orphelines dans Storage, initialement listé dans l'audit comme point à reconfirmer pour ce cycle, était déjà résolu par le Cycle A (réutilisation de `durableImageUrls` au lieu de ré-uploader à chaque sauvegarde) — aucune action supplémentaire nécessaire.

Non vérifié en conditions réelles (mêmes limitations d'accès que le Cycle A). Vérifié : typecheck, lint, build, 195 tests.

### 4.2 Génération d'annonce par IA

```
UploadStep (photos) → GeneratorPage (état du flow)
  ...
  → lib/storage.ts::uploadListingPhotos() envoie les blob: URL vers le bucket
    Storage `listing-photos` (public, scopé par dossier `{user_id}/`)
  → insert dans `listings` (image_urls = URLs durables, pas les blob: d'origine ;
    status: 'draft' ou 'en_stock' selon l'action de l'utilisateur)
```
...
**Correctif du 2026-07-13 — les photos ne survivaient pas à un reload.** Avant cette date, `image_urls` était rempli directement avec les `blob:` URL du navigateur (jamais uploadées nulle part — zéro appel `supabase.storage` dans tout le code) : valides seulement le temps de l'onglet d'origine, cassées dès une nouvelle session. `uploadListingPhotos()` corrige ça en uploadant chaque photo vers Storage avant l'`insert`, et échoue explicitement (bloque la sauvegarde) plutôt que de retomber silencieusement sur l'URL cassée.

**Le sélecteur de style de fond (`PHOTO_STYLES`) et la case "Amélioration automatique" ne font toujours rien** — confirmé en auditant `analyze-clothing/index.ts` : ces deux réglages ne sont interpolés que dans le **prompt texte** envoyé à Gemini (`gemini-2.5-flash`, un modèle d'analyse texte/vision, pas de génération d'image). Aucune image différente n'est jamais produite. Plutôt que de laisser croire que ça fonctionne, `UploadStep.tsx` affiche désormais ces réglages comme désactivés avec un badge "Bientôt disponible" — une vraie génération d'image nécessiterait une API dédiée (modèle image de Gemini ou équivalent), un nouveau pipeline de stockage, et un coût par appel : un chantier à part entière, pas fait cette phase.

**Architecture préparée sans implémentation (sprint extension V1, 2026-07-13)** : `src/lib/photoGeneration/` pose l'interface qu'un futur vrai fournisseur devra implémenter (`PhotoGenerationProvider.generate(request): Promise<{ previewUrls }>`, `PhotoStyle` = les 4 valeurs déjà affichées), avec pour seule implémentation `notImplementedProvider` (lève une erreur explicite si jamais appelé). Zéro changement visible : `UploadStep.tsx` reste exactement dans l'état "Bientôt disponible" ci-dessus, ce module n'est appelé par aucun code pour l'instant — but unique, permettre de brancher une vraie API plus tard derrière cette même interface sans toucher au Générateur ni à `EditListingModal.tsx`.
...
**Parcours Scanner — corrections critiques (audit pré-bêta, 2026-07-24)** : l'audit de bout en bout a confirmé deux bugs réels, tous deux corrigés :
- **Échec silencieux d'un scan planifié (cron)** : `logProgress`/`writeTerminal` dans `scripts/vinted-scan.ts` ne faisaient rien sans `ACTION_ID` (jamais fourni par le cron), donc un run cron qui échouait après le `DELETE FROM market_opportunities` en début de script ne laissait absolument aucune trace — un bêta-testeur voyait "Aucune opportunité", indiscernable d'un simple manque de données. Nouvelle table `scan_runs` (migration `20260724160000_add_scan_runs.sql`), écrite pour **chaque** run (cron ou manuel, contrairement à `action_log` qui est per-user et réservé aux scans manuels) : `Opportunities.tsx` affiche désormais "Dernier scan réussi : il y a Xh" et un bandeau d'avertissement explicite si le dernier run a échoué.
- **Course entre le timeout client (6 min) et l'écriture terminale du script** : au-delà de `TERMINAL_WAIT_TIMEOUT_MS`, le client marquait la ligne `action_log` `status='error'` alors que le job GitHub Actions pouvait très bien tourner encore et écrire son propre statut plus tard (`service_role`, bypasse RLS) — sans notification du "flip". Plus grave : `checkNoScanInProgress` (`checks.ts`) ne regardait que `status='pending_confirmation'`, donc ce timeout levait à tort le garde-fou anti-double-scan, permettant de lancer un **second scan concurrent** pendant que le premier tournait toujours (risque réel de collision sur le `DELETE`/`upsert` de `market_opportunities`). Corrigé sans toucher au système de types `ActionOutcome` (pas de nouveau statut générique, blast radius minimal) : `SCAN_TIMEOUT_ERROR_MESSAGE` (`scanMarket.ts`) est désormais une constante exportée, et `useActionEngine.ts::prepareAction()` considère un scan comme "en cours" si sa ligne `action_log` a soit `status='pending_confirmation'`, soit `status='error'` avec exactement ce message — qui disparaît naturellement dès que le vrai statut terminal est écrit.
...
**KPI "stock actif" vs liste visible (correctif 2026-07-13)** : `status !== 'vendu'` seul comptait à tort les articles `reserved`/`sold_pending` côté Vinted comme du stock disponible (`deriveResellOsStatus`, `extension/src/background/sync.ts`, ne bascule `status` sur `'vendu'` qu'à `sold_completed` — un article réservé reste `en_stock` en interne). `src/lib/listingStatus.ts::isActivelyInStock()` exclut en plus `reserved`/`sold_pending` des totaux "Articles en stock"/"Valeur du stock"/"Investissement"/"Marge potentielle" (`StockPage.tsx`, `DashboardHome.tsx`) — la liste détaillée et les compteurs "Statut Vinted" continuent d'afficher ces articles avec leur badge réel, seuls les agrégats "stock actif" changent.

**Édition d'une annonce existante (`EditListingModal.tsx`, 2026-07-13)** : premier chemin `UPDATE` sur une ligne `listings` déjà persistée, déclenché depuis `StockPage.tsx` — jusqu'ici seule la création (Générateur) écrivait dans cette table. Édition en place uniquement (pas de versions multiples), limitée aux champs propriétaire de l'app pour ne jamais entrer en conflit avec la synchro extension. Détail complet dans [EXTENSION.md](EXTENSION.md) §6.2bis.
...
→ DashboardHome.tsx ("Copilote")       narrations + signal dominant cliquable ; "Annonces récentes"
                                       affiche aussi le One Score de chaque annonce
→ StockPage.tsx / Opportunities.tsx    badge de recommandation + composant partagé OneScoreBar
                                       ("One Score X/100", src/components/ui/OneScoreBar.tsx) —
                                       un seul nom, un seul composant, jamais "Score IA" ni
                                       "Opportunity Score" (audit Brand Book, 2026-07-23)
```
...
**`edit_listing` (sprint extension V1, 2026-07-13) est la deuxième action réelle** — `handlers/editListing.ts` (app) + `handlers/editListing.ts` (extension), même structure exacte que `publish_listing` (checks + buildPreview seulement, `deps.runViaExtension()`, même `PublishStep`/port de progression réutilisés tels quels). Diffère uniquement par la cible (`vinted.fr/items/{id}/edit` au lieu de `/items/new`) et par un payload plus restreint (texte/attributs, sans photos ni taille de colis — limite V1 assumée, voir EXTENSION.md §6.2bis). La logique de remplissage de formulaire (catégorie, correspondance d'options, vérification du compte connecté) a été extraite de `vinted-publish.ts` vers `extension/src/content/formFill.ts` au moment où ce deuxième appelant réel est apparu — pas avant, pour ne pas abstraire sur la base d'un seul usage.
...
**Pagination (2026-07-23, P0 #8 de l'audit pre-lancement)** : la requête `action_log` n'avait initialement aucun `.limit()`/`.range()` — un historique grandissant indefiniment avec l'usage reel. `useActionHistory.ts` charge desormais par pages de 30 (`PAGE_SIZE`), avec un bouton "Charger plus" (`loadMore`) dans `ActionsPage.tsx`. Les 3 compteurs (total/succes/erreurs) viennent de requetes `count: 'exact', head: true` separees (`loadCounts`), jamais de `rows.length`, pour rester exacts au-dela de la premiere page. L'abonnement Realtime re-synchronise desormais via `refresh()` (qui recharge autant de lignes que deja affichees, pas juste la premiere page) plutot que `load()`, pour ne pas faire regresser silencieusement la pagination de l'utilisateur a chaque evenement. Limite connue et assumee : la recherche texte reste cote client sur les lignes deja chargees (jamais de requete reseau par frappe), donc ne trouve pas une action au-dela de ce qui a ete charge — un vrai filtre serveur reste un chantier separe si besoin.

**Pagination de Stock (2026-07-24, meme P0 #8, decision produit validee)** : `StockPage.tsx` applique un fenetrage purement cote client (`visibleCount`/`STOCK_PAGE_SIZE = 30`, bouton "Charger plus") sur `filtered`, sans deuxieme requete reseau — choix delibere de l'utilisateur ("simple, previsible, facile a maintenir", pas d'infinite scroll pour un outil de gestion). `items` (le fetch complet) reste inchange et continue d'alimenter les StatCards (valeur du stock, marge, ROI moyen, etc.), qui doivent rester des totaux exacts, jamais bornes par `visibleCount`. **Dashboard/AccountingPage/StatsPage restent volontairement non pagines pour la beta** (decision produit du 2026-07-24) : leurs requetes `listings`/`expenses` alimentent des agregats financiers calcules cote client — y ajouter une limite corromprait silencieusement ces totaux, un risque juge pire que la lenteur actuelle tant que le volume reel reste faible. A reevaluer si le volume de donnees des utilisateurs rend cela mesurablement un probleme (agregation cote serveur alors necessaire, pas juste une pagination d'affichage).
...
**Limite honnête de cette validation** : la revalidation ci-dessus a été faite en isolant les cas problématiques réels avec des tests unitaires ciblés — pas en rejouant un scan complet avec le nouveau moteur déployé, puisqu'aucun commit n'avait encore été poussé sur `origin/main` au moment du scan analysé (l'Action GitHub exécute ce qui est sur la branche par défaut, pas les changements locaux). Une simulation locale reconstituant le pool de comparables à partir de `market_opportunities` (déjà filtré) a été tentée puis écartée : le pool reconstruit est biaisé (il ne contient que des items déjà sélectionnés comme bonnes affaires), ce qui fausse le prix de marché recalculé. La validation définitive du taux de sélectivité réel ("20 excellentes" vs. l'ancien "329 moyennes") nécessite un nouveau scan réel après déploiement du code.

### 4.8.4 `npm ci` échouait en CI (2026-07-12) — lockfile désynchronisé, sans lien avec le moteur

Après déploiement du code (commit `a45e3e9`), le premier scan réel déclenché via `workflow_dispatch` a échoué en 9 secondes à l'étape `npm ci`, avant même d'atteindre le scan lui-même. Cause réelle (pas une supposition) : l'arbre `esbuild` imbriqué sous `tsx` (`~0.28.0`) était **incomplet** dans `package-lock.json` — certaines variantes de plateforme présentes, la plupart absentes — un état que `npm install` tolère silencieusement (d'où l'absence de tout signal en local) mais que `npm ci` (utilisé par GitHub Actions, mode strict) rejette. Corrigé par un reinstall complet (`node_modules` + lockfile supprimés, `npm install` propre) — `package.json` inchangé, aucune dépendance directe modifiée. Vérifié réel : `npm ci` réussi sur les deux packages du dépôt (racine et `extension/`) avant de repousser (commit `2d4798a`).

### 4.8.5 `market_price_observations` restait vide malgré un scan réussi (2026-07-12) — diagnostic et correctif

Le scan relancé après le correctif ci-dessus a réussi et enregistré 214 opportunités réelles dans `market_opportunities` — mais `market_price_observations` est restée à **0 ligne**, sans exception ni message d'erreur visible dans le log. Diagnostic mené avant toute correction (pas de modification à l'aveugle) :

- **RLS écartée avec preuve** : `pg_policies` interrogé en direct ne montre qu'une policy `SELECT` pour `authenticated` sur cette table, aucune policy `INSERT` — mais `service_role` (utilisé par `scripts/vinted-scan.ts`, confirmé par le succès de l'écriture dans `market_opportunities` via le même client) bypass RLS par construction dans Postgres/Supabase, indépendamment des policies déclarées. RLS ne peut donc pas être la cause.
- **Migration écartée avec preuve** : `information_schema.columns` interrogé en direct confirme un schéma complet et conforme à la migration (`price numeric not null`, `vinted_url text not null`, `watchlist_id`/`brand`/`category`/`favourites` nullables comme prévu, `scanned_at` avec défaut `now()`).
- **Hypothèse retenue, non prouvée à 100% faute de log assez détaillé sur ce run** : `price` est `not null`, et `JSON.stringify(NaN)` devient silencieusement `null` — une seule annonce scrapée avec un prix non fini (édge case de parsing) aurait suffi à faire échouer l'insert **multi-lignes complet** (~3000-4000 lignes en un seul appel), un insert Postgres étant atomique par lot. Le code précédent avalait cette erreur avec un `console.error` correct mais qui n'était pas visible/assez explicite dans les logs analysés.
...
### 4.8.7 Badge de verdict et explications concrètes (2026-07-12) — 100% dérivé du moteur existant

Objectif produit : transformer les métriques déjà calculées (§4.8) en recommandation d'achat lisible, sans construire de nouveau moteur de scoring. `src/lib/opportunityVerdict.ts` (frontend uniquement, aucun changement de `scripts/opportunity-engine/` requis) dérive un verdict unique (`excellent` 🔥 / `recommande` 🟢 / `a_surveiller` 🟡 / `trop_risque` 🔴) à partir de `score`/`confidence`/`risk_level`, déjà exposés sur `MarketOpportunity` :

- `risk_level === 'eleve'` → toujours `trop_risque`, quel que soit le score.
- Sinon, le gate existant (`MIN_SCORE_FOR_OPPORTUNITY=65`, `MIN_CONFIDENCE_FOR_OPPORTUNITY=50`, `scripts/opportunity-engine/constants.ts`) détermine `a_surveiller` vs validé.
- Le seuil "excellent" (`score>=85 && confidence>=70 && risk==='faible'`) n'est pas nouveau — il reprend exactement la condition déjà codée dans `scripts/opportunity-engine/explanation.ts:38` pour la mention "Très forte probabilité de revente rentable". Les 4 nombres du fichier sont dupliqués (pas importés) car `scripts/` et `src/` sont deux projets TypeScript séparés (`tsconfig.scripts.json`/`tsconfig.app.json`) — même précédent que `OpportunityRiskLevel`, déjà dupliqué entre `scripts/opportunity-engine/types.ts` et `src/lib/types.ts`.

Le badge remplace l'ancien badge `TIERS` (palier ROI seul) sur `OpportunityCard`, même position et style de pastille — aucune refonte de mise en page. `OpportunityFilterPanel.tsx` gagne une rangée de filtre par verdict, même pattern visuel que le filtre de risque déjà existant.

**Section "Pourquoi cette opportunité ?" enrichie de chiffres concrets** : le breakdown du moteur reste en libellés par palier ("ROI élevé (≥100%)") — utile pour la nuance, mais jamais les vraies valeurs de l'opportunité affichée. `OpportunityCard` calcule désormais, au rendu, 4 lignes à chiffres réels à partir de champs déjà exposés (`price_found`, `market_price`, `roi`, `competing_listings_count`, `resale_days_min/max`) : aucun nouveau calcul serveur, aucune migration. Un seul ajout côté moteur, minimal et justifié : `confidence.ts` confirme désormais explicitement la stabilité du prix ("Prix stable sur les annonces comparables", `delta: 0`, purement informatif) quand la dispersion est sous `PRICE_DISPERSION_MODERATE` — avant cette passe, l'absence de pénalité restait silencieuse, privant l'utilisateur du signal "stabilité du prix" qu'il demande explicitement.

### 4.8.8 Performance à l'échelle — index manquant sur le vrai filtre de requête (2026-07-12)

`scripts/vinted-scan.ts` filtre déjà `market_price_observations` sur une fenêtre glissante de 60 jours (`.gte("scanned_at", ...)`, `OBSERVATION_LOOKBACK_DAYS`), mais aucun des 3 index existants (`vinted_url`, `watchlist_id`, `category` — tous avec `scanned_at` en second, jamais en tête) ne permet à Postgres un balayage d'index sur ce filtre. Sans index dédié, la requête fait un balayage séquentiel complet dont le coût grandit avec le volume TOTAL de la table, pas seulement la fenêtre de 60 jours — le vrai risque à l'échelle de plusieurs dizaines de milliers d'observations. Nouvel index `market_price_observations_scanned_at_idx (scanned_at desc)` (migration `20260712130000`).

Complété par un plafond défensif documenté (pas un chiffre choisi au hasard) : `.order("scanned_at", { ascending: false }).limit(20000)` sur la même requête — un seul scan réel a déjà produit ~3400 observations, et un cron toutes les 4h sans purge sur 60 jours représente un ordre de grandeur de plusieurs centaines de milliers de lignes sans plafond. L'ordre par date décroissante garantit que seules les observations les plus anciennes de la fenêtre seraient tronquées si la limite était atteinte, jamais les plus récentes. Le regroupement en mémoire (`context.ts:buildScanContext`) reste `Map`-based et O(n), sans boucle imbriquée — aucun changement nécessaire là.
...
**Corrections de friction (audit pré-bêta, 2026-07-24)**, au-delà des deux bugs critiques du §4.3 ci-dessus :
- `WatchlistPage.tsx::handleSubmit()` fermait la modale d'ajout/édition inconditionnellement, même après un échec réel d'écriture — l'utilisateur ne voyait qu'un bandeau d'erreur en arrière-plan, facile à rater pendant l'animation de fermeture. `useWatchlist.ts` (`addEntry`/`updateEntry`/`toggleActive`/`deleteEntry`) retourne désormais un booléen de succès réel, consommé par l'appelant.
- Ajouter une recherche déjà suivie (contrainte unique `(user_id, brand, model)`) renvoyait un message d'erreur générique — `useWatchlist.ts` détecte maintenant le code Postgres `23505` (violation d'unicité) pour un message explicite ("Tu suis déjà cette marque et ce modèle.").
- Aucun lien ni pédagogie n'existait entre la Watchlist et les Opportunités : un nouveau bêta-testeur ajoutait une recherche puis voyait un écran "Aucune opportunité, lance un scan ou modifie tes filtres" sur `Opportunities.tsx`, qui suggérait à tort un problème de filtres plutôt qu'un simple manque de données pas encore scannées. `WatchlistPage.tsx` affiche désormais un bandeau explicatif (cadence du scan, lien direct vers Opportunités) et `Opportunities.tsx` distingue "aucune donnée du tout" de "des données existent mais tes filtres les excluent toutes".
- `failedSearches` (recherches individuellement ignorées après échec de navigation Vinted) était déjà renvoyé par `vinted-scan.ts` dans `resultPayload` mais jamais lu côté client — `scanNow()`/`ScanProgressModal.tsx` le surfacent maintenant après un scan manuel.
- `OpportunityCard` (inline dans `Opportunities.tsx`) affichait "0 €" pour un prix `null` (`Number(null) → 0`) au lieu d'une valeur explicitement inconnue — corrigé pour ne jamais fabriquer un chiffre sur une donnée absente, cohérent avec le principe déjà établi ailleurs dans l'app (Cycle B pré-bêta, `src/lib/errorMessages.ts`). Image cassée sans repli visuel et lien "Voir l'annonce" rendu inerte sans distinction visuelle quand `vinted_url` est absent, également corrigés.

### 4.10 Espace Communauté (`src/pages/dashboard/CommunityPage.tsx`, 2026-07-27)

Nouvel onglet `'community'` du dashboard : hub intégré (Nouveautés/Changelog, Tutoriels, Guides, Ressources, Roadmap publique, Sondages, Suggestions, FAQ, Support par tickets, Discord) construit en 7 lots séquentiels, chacun vérifié et poussé indépendamment (voir l'historique git, `feat(community): Lot 1` à `Lot 6`). Discord reste la couche de discussion temps réel — cet espace est la couche produit, contrôlée et archivée.

**Décision de navigation** : un seul nouvel item `DashboardPage = 'community'` dans la sidebar existante, avec une tab bar interne (`CommunityTab`, calquée sur `SettingsTab`/`SettingsPage.tsx`) plutôt qu'une refonte de la sidebar en groupes — évite de toucher un composant central utilisé par tout le dashboard pour une fonctionnalité qui n'en a pas besoin. Même rendu conditionnel strict qu'ailleurs (jamais de montage caché d'un onglet inactif), important une fois que plusieurs onglets portent un canal Realtime.

**Modèle de rôle/palier** : `profiles.role` (`'user'`/`'admin'`) reste la seule distinction *administrative* (publier, modérer). `profiles.plan` (`'free'`/`'pro'`/`'team'`) est déjà la distinction de *palier d'accès* — les rôles "Membre/Client/Client Premium" demandés par l'utilisateur pour le futur se mappent directement sur les valeurs de `plan` existantes, pas besoin d'élargir `role`. Chaque table de contenu (`community_content`, `roadmap_items`, `polls`, `suggestions`, `support_tickets`) porte une colonne `min_plan` nullable en tolérance de schéma — toujours `null` (aucune restriction) tant qu'un vrai gating par palier n'est pas demandé.

**`is_admin()`** (fonction SQL partagée, `20260727100000_add_community_content.sql`) centralise la vérification `role = 'admin'` réutilisée par ~20 policies d'écriture à travers les 9 tables — évite qu'une des copies contienne une faute qui affaiblisse silencieusement une policy. `useIsAdmin()` (`src/hooks/useIsAdmin.ts`) est l'équivalent frontend, réutilisé par ce chantier et rétrofité sur les 3 sites qui dupliquaient `profile?.role === 'admin'` avant ce lot (`StockPage.tsx`, `DashboardHome.tsx`, `GeneratorPage.tsx`) — **commodité d'affichage uniquement, la RLS reste la seule vraie frontière de sécurité** dans les deux cas.

**7 tables nouvelles** (détail RLS complet dans [DATABASE.md](DATABASE.md)) :
- `community_content` — unifie changelog/tutorial/guide/resource/faq (même forme éditoriale : titre/corps/brouillon-publié/auteur/date de publication), colonnes `category`/`sort_order` dès le départ pour le regroupement Notion-like, `resource_kind`/`resource_url` nullables pour les ressources, `discord_channel_id`/`discord_message_id`/`discord_synced_at` nullables en tolérance de schéma pour une future synchronisation Discord (non construite dans ce chantier).
- `roadmap_items` — cycle de vie `planned`/`in_progress`/`shipped` (pas brouillon/publié), colonnes Now/Next/Later côté UI (`RoadmapTab.tsx`).
- `polls`/`poll_options`/`poll_votes` — un vote par utilisateur par sondage (`unique(poll_id, user_id)`), `poll_options.votes_count` dénormalisé maintenu par le trigger `bump_poll_option_votes_count` (`SECURITY DEFINER`, contourne le fait qu'un votant normal n'a aucune policy `UPDATE` directe sur `poll_options`). **Premier usage de Supabase Realtime hors Centre des Actions** (`usePolls.ts`) : les résultats se mettent à jour en direct même si le vote vient d'un autre onglet/utilisateur. `poll_votes` n'est jamais lisible publiquement (seul son propre vote, ou l'admin).
- `suggestions`/`suggestion_votes` — **seul endroit de tout l'espace Communauté où la création n'est pas réservée à l'admin** (`insert_own_suggestions` ne vérifie que `author_id = auth.uid()`, jamais `is_admin()` — commenté explicitement dans la migration pour qu'un lot futur ne le gate pas par erreur). Seuls le statut et la réponse officielle (`admin_reply`) sont admin-only. Tri par `votes_count desc` (pas par date) : une suggestion pertinente mais ancienne reste visible si elle est plus votée qu'une suggestion récente isolée. Même mécanique de compteur dénormalisé + trigger `SECURITY DEFINER` que les sondages.
- `support_tickets`/`ticket_messages` — **lot le plus sensible côté confidentialité** : strictement privé (propriétaire + admin uniquement, jamais de lecture publique contrairement au reste de l'espace). Garde-fou anti-usurpation vérifié en base (`insert_own_ticket_messages`) : `is_admin_reply` ne peut valoir `true` que si le posteur est réellement `is_admin()`, empêchant un utilisateur de faire passer son propre message pour une réponse officielle de l'équipe. `ticket_messages` est un journal immuable (pas d'update/delete), même discipline que `action_log_entries`.

**Pattern de hook** identique à `useWatchlist.ts` partout : `{ items, loading, error, mutations retournant Promise<boolean>, refresh }`. `CommunityContentEditorModal.tsx` (admin, create/edit) est réutilisée telle quelle pour les 5 types de `community_content` (Lots 1-2) ; Roadmap/Sondages/Suggestions ont chacun leur propre modale dédiée, leur forme de données étant trop différente d'un post éditorial.
...
- **Stock** : vue unifiée (fusion `listings`/`vinted_listings`, 2026-07-09) — suivi des annonces (brouillon/en stock/vendu), marquage de vente manuel ou automatique sur détection Vinted, marge/ROI quand le prix d'achat est connu, statuts Vinted réels (en ligne/réservé/vente en cours/vendu/brouillon/masqué) filtrés par compte actif, synchronisation à la demande, KPI "stock actif" excluant les articles réservés/en cours de vente (§4.4) — voir [EXTENSION.md](EXTENSION.md) §5
- **Édition d'une annonce** : modale dédiée depuis Stock (titre/description/prix/photos, relance de l'analyse IA sur les mêmes photos), enregistrement en place, enchaînement optionnel sur la publication si l'annonce n'est pas encore en ligne — voir [EXTENSION.md](EXTENSION.md) §6.2bis
