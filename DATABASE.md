...
| `action_log` | **Journal du Action Engine — préparation Phase 3** (2026-07-10) : une ligne par action déclenchée via `src/lib/actions/` (`kind`, `vinted_account_id`, `listing_id`, `status` `pending_confirmation`→`success`/`error`/`cancelled`/`not_implemented`, `preview`, `result_payload`/`error_message`, `duration_ms`). Remplace la conception `sync_jobs` jamais implémentée. Écrite uniquement par l'app web (insert dans `prepare()`, update dans `confirm()`/`cancel()`) — l'extension n'y écrit pas cette phase. Voir `EXTENSION.md` §5 et `ARCHITECTURE.md` §4.6 | `auth.uid() = user_id`, select/insert/update (pas de delete) ; l'insert valide en plus que `vinted_account_id`/`listing_id` appartiennent à l'utilisateur |
