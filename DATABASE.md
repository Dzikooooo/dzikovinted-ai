...
| `action_log` | **Journal du Action Engine** (2026-07-10) : une ligne par action déclenchée via `src/lib/actions/` (`kind`, `vinted_account_id`, `listing_id`, `status` `pending_confirmation`→`success`/`error`/`cancelled`/`not_implemented`, `current_step`, `preview`, `result_payload`/`error_message`, `duration_ms`). Remplace la conception `sync_jobs` jamais implémentée. Écrite uniquement par l'app web (insert dans `prepare()`, update dans `confirm()`/`cancel()`) — l'extension n'y écrit pas. Lue en temps réel par le Centre des Actions via Supabase Realtime (table ajoutée à la publication `supabase_realtime`). Voir `EXTENSION.md` §5 et `ARCHITECTURE.md` §4.6/§4.7 | `auth.uid() = user_id`, select/insert/update (pas de delete) ; l'insert valide en plus que `vinted_account_id`/`listing_id` appartiennent à l'utilisateur |
| `action_log_entries` | **Journal détaillé du Centre des Actions** (2026-07-10) : append-only, une ligne par étape/message journalisé pour une action (`action_id`, `step`, `message`, `at`) — permet de rejouer l'historique complet d'une action après coup, la progression du port `action-progress` (§EXTENSION.md §6.0) étant sinon purement éphémère. Même discipline que `listing_metric_snapshots`. Écrite uniquement par l'app web (`useActionEngine.ts`), lue en temps réel par le Centre des Actions | `action_id in (select id from action_log where user_id = auth.uid())`, select+insert seulement (pas de update/delete, c'est un journal) |
