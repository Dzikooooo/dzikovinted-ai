...
| `listings` | Annonces générées/en stock/vendues | `auth.uid() = user_id`, CRUD complet |
...
| `vinted_listings` | Miroir des annonces Vinted (synchronisées par l'extension), scopées par `vinted_account_id` — distinct de `listings`. Colonnes `brand`/`size` ajoutées (2026-07-09). `status` : `online`/`reserved`/`sold_pending`/`sold_completed`/`draft`/`hidden`/`deleted`/`unknown`, pas de contrainte CHECK. `deleted` est une **suppression douce** (mise à jour de statut, pas de `DELETE`) — toute lecture doit filtrer `status != 'deleted'` explicitement. Voir `EXTENSION.md` §5 | RLS via `vinted_account_id in (select id from vinted_accounts where user_id = auth.uid())` — pas de `user_id` sur cette table, voir `EXTENSION.md` §5 pour la justification de cet écart |
