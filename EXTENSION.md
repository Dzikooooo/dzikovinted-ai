...
**État d'implémentation** : la Phase 1 complète (1.1 appairage, 1.2 détection du compte, 1.3 synchronisation des annonces) est codée **et validée en conditions réelles**. La refonte multi-comptes est ensuite lancée : **Phase A (fondation schéma `vinted_accounts`) faite et validée**, sans régression visible. **Phase B (interface multi-comptes) faite et validée en conditions réelles** avec deux comptes réels (`alexisdzk`, `matleshop`) : sélecteur de compte premium (`AccountSwitcher`), gestion complète dans Paramètres (renommage, compte par défaut via RPC, suppression), Dashboard et page Compte Vinted en double mode (vue globale/filtrée). Pendant cette validation, l'utilisateur a découvert que la synchronisation des annonces (étape 1.3) était incomplète et mal étiquetée depuis le début — **corrigé le 2026-07-09**, voir §5 `vinted_listings`. **Page Stock fiabilisée et rattachée par compte (2026-07-09)** : nouvel onglet "Vinted" dans `StockPage.tsx`, miroir complet par compte avec taxonomie de statut stable (`online`/`reserved`/`sold_pending`/`sold_completed`/`draft`/`hidden`/`deleted`/`unknown`), suppression douce, CA confirmé/en attente, synchronisation à la demande sans nouvelle permission Chrome — voir §5 et §6.2. Le rattachement bénéfice/ROI réel (lien `listings` ↔ `vinted_listings`, prix d'achat) reste hors scope, différé à une Phase C future — voir `ROADMAP.md`. Plusieurs décisions ci-dessous ont été révisées par rapport à la conception initiale, certaines après vérification directe du schéma existant, d'autres après des bugs réels découverts en test live (§3, §5) — marquées explicitement.
...
### `vinted_listings` (implémentée, étape 1.3, migrée en Phase A, synchronisation refondue le 2026-07-09, taxonomie de statut et suppression douce le 2026-07-09)

Les annonces synchronisées depuis Vinted ne vont **pas** dans la table `listings` existante : `listings` modélise des annonces *créées dans ResellOS* (générées par IA, avec prix d'achat/frais/statut de vente — des champs qu'une annonce Vinted brute scrapée n'a pas). `vinted_listings` est un miroir de ce qui est réellement en ligne sur Vinted (`vinted_account_id`, `vinted_item_id`, `title`, `price`, `image_url`, `vinted_url`, `status`, `brand`, `size`, `favourites`, `views`), réécrit à chaque visite du profil.

**Bug de synchronisation incomplète trouvé et corrigé (2026-07-09)** : l'implémentation initiale (étape 1.3) lisait le DOM une seule fois au chargement de la page, via `extractListingCards()` sur les cartes `product-item-id-*` déjà rendues — sans jamais déclencher le défilement infini de Vinted, ni gérer le changement d'onglet Actifs/Vendus (un état client, pas une navigation), ni écrire de statut réel (`status` était fixé en dur à `'actif'` pour toute ligne). Résultat vérifié en conditions réelles : alexisdzk (40 annonces actives réelles) n'en synchronisait que 20 ; matleshop ne synchronisait que les annonces visibles sur l'onglet actif au moment du chargement, toutes mal étiquetées `'actif'`.

**Correction** : en observant le trafic réseau du propre frontend de Vinted pendant un défilement, l'extension utilise maintenant l'API REST same-origin que Vinted appelle lui-même : `GET https://www.vinted.fr/api/v2/wardrobe/{vinted_user_id}/items?page=N&per_page=50&order=relevance` (`extension/src/content/wardrobeApi.ts`). Ce n'est pas un contournement anti-bot — c'est exactement la requête émise par la page Vinted elle-même au scroll, consommée directement plutôt que simulée. La pagination suit `pagination.total_pages`/`total_entries` renvoyés par Vinted jusqu'à épuisement complet (aucune limite arbitraire). L'ancien scraping DOM de cartes (`extractListingCards`/`RawListingCard`) est supprimé de `selectors.ts`.

**Taxonomie de statut (2026-07-09)** : chaque annonce porte des booléens Vinted (`is_draft`, `is_closed`, `is_reserved`, `is_hidden`, `is_processing`) normalisés en un statut texte stable : `online` / `reserved` / `sold_pending` (jamais observé à `true` en conditions réelles sur les deux comptes de test, conservé au cas où) / `sold_completed` / `draft` / `hidden` / `deleted` / `unknown` (item sans titre exploitable — remonté plutôt que silencieusement ignoré). Pas de contrainte CHECK, reste flexible. `brand`/`size` sont des champs texte simples côté API Vinted (pas d'objet imbriqué), `size` peut être une chaîne vide (traitée comme `null`).

**Suppression douce** : `recordListings()` (`extension/src/background/sync.ts`) traite chaque scan complet comme un **miroir** — upsert des lignes présentes (avec `brand`/`size`/`status`), puis `UPDATE status = 'deleted'` (pas de `DELETE` physique) sur toute ligne du compte absente du scan courant et pas déjà `'deleted'`. Préserve l'historique (ventes passées, statistiques) au lieu de le perdre. Tout code lisant `vinted_listings` doit filtrer `status != 'deleted'` explicitement (fait dans `DashboardHome.tsx`, `VintedAccountPage.tsx`, `StockPage.tsx`).

Validé en conditions réelles sur les deux comptes de test après la correction : alexisdzk 40 online + 1 reserved + 3 sold_completed = 44 (contre 20 avant, tous mal étiquetés) ; matleshop 4 online + 10 sold_completed = 14 (contre 14 tous mal étiquetés `'actif'` avant) — comptes exacts confirmés par l'utilisateur sur ses vraies annonces Vinted, `brand` peuplé sur 100% des lignes.

**Réserve non résolue** : l'API `wardrobe/items` a été testée à la fois en anonyme (40 items actifs uniquement, aucun booléen à `true`) et via une vraie session propriétaire authentifiée (statuts variés, confirmé ci-dessus) — le comportement attendu (l'API renvoie plus de champs/annonces à l'utilisateur authentifié) est donc confirmé en conditions réelles, pas seulement supposé par analogie avec d'autres marketplaces. `sold_pending` (`is_processing`) reste théorique : jamais observé à `true` sur les deux comptes de test.
