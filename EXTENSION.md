...
### `vinted_listings` (implémentée, étape 1.3, migrée en Phase A, synchronisation refondue le 2026-07-09)

Les annonces synchronisées depuis Vinted ne vont **pas** dans la table `listings` existante : `listings` modélise des annonces *créées dans ResellOS* (générées par IA, avec prix d'achat/frais/statut de vente — des champs qu'une annonce Vinted brute scrapée n'a pas). `vinted_listings` est un miroir de ce qui est réellement en ligne sur Vinted (`vinted_account_id`, `vinted_item_id`, `title`, `price`, `image_url`, `vinted_url`, `status`, `favourites`, `views`), réécrit à chaque visite du profil.
...
**Correction** : en observant le trafic réseau du propre frontend de Vinted pendant un défilement, l'extension utilise maintenant l'API REST same-origin que Vinted appelle lui-même : `GET https://www.vinted.fr/api/v2/wardrobe/{vinted_user_id}/items?page=N&per_page=50&order=relevance` (`extension/src/content/wardrobeApi.ts`). Ce n'est pas un contournement anti-bot — c'est exactement la requête émise par la page Vinted elle-même au scroll, consommée directement plutôt que simulée. La pagination suit `pagination.total_pages`/`total_entries` renvoyés par Vinted jusqu'à épuisement complet (aucune limite arbitraire), et chaque annonce porte des booléens (`is_draft`, `is_closed`, `is_reserved`, `is_hidden`, `is_processing`) normalisés en un statut texte (`actif`/`vendu`/`vendu_non_finalise`/`reserve`/`masque`/`brouillon`). L'ancien scraping DOM de cartes (`extractListingCards`/`RawListingCard`) est supprimé de `selectors.ts`.

Le content script récupère désormais la totalité des annonces du compte à chaque visite (tous statuts confondus) : `recordListings()` (`extension/src/background/sync.ts`) traite donc l'ensemble reçu comme un **miroir complet** — upsert des lignes présentes, puis suppression de toute ligne `vinted_listings` du compte dont le `vinted_item_id` n'apparaît plus dans le scan (annonce supprimée ou déplacée sur Vinted).

Validé en conditions réelles sur les deux comptes de test après la correction : alexisdzk 40 actif + 1 réservé + 3 vendu = 44 (contre 20 avant, tous mal étiquetés) ; matleshop 4 actif + 10 vendu = 14 (contre 14 tous mal étiquetés `'actif'` avant) — comptes exacts confirmés par l'utilisateur sur ses vraies annonces Vinted.

**Réserve non résolue** : l'API `wardrobe/items` a été testée à la fois en anonyme (40 items actifs uniquement, aucun booléen à `true`) et via une vraie session propriétaire authentifiée (44/14 items avec statuts variés, confirmé ci-dessus) — le comportement attendu (l'API renvoie plus de champs/annonces à l'utilisateur authentifié) est donc confirmé en conditions réelles, pas seulement supposé par analogie avec d'autres marketplaces.
