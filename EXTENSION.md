...
### `sku` — numérotation automatique (sprint extension V1, 2026-07-13)

Troisième axe de propriété de champ, distinct des deux règles ci-dessus (« toujours rafraîchi » / « fixé à la création ») : `sku` est **assigné une seule fois, jamais rafraîchi, jamais recalculé** — ni par la synchro en masse (`recordListings`), ni par l'import intelligent (`recordSingleItemImport`, §6.1bis, qui exclut explicitement `sku` de son payload de mise à jour), ni par une édition manuelle (`EditListingModal.tsx`). Attribué côté DB par un trigger `BEFORE INSERT` (`assign_sku_before_insert` → `allocate_next_sku`, voir `DATABASE.md`) dès que `sku` est `null` à l'insertion — plus petit entier positif libre parmi les annonces **actives** de l'utilisateur (une vente ou une suppression douce libère implicitement le numéro). Numérotation **unique par utilisateur**, pas par compte Vinted : `alexisdzk` et `matleshop` partagent la même suite de numéros.

Le titre stocké en base ne contient jamais le SKU — `src/lib/sku.ts::formatTitleWithSku(title, sku)` ne l'ajoute (`"Titre #12"`) qu'au moment de construire un payload à destination de Vinted (publication §6.1, modification §6.2bis). À l'import (§6.1bis), `extractSkuFromTitle` fait l'inverse : si le titre Vinted importé se termine déjà par `#N`, ce numéro est repris tel quel comme `sku` explicite plutôt que d'en laisser un nouveau être alloué — évite de perdre une numérotation manuelle préexistante lors du tout premier import d'un article ancien.
...
**Message utilisateur en cas de timeout (Cycle B pré-bêta, 2026-07-24)** : `domWait.ts::describeTimeout()` traduit un `WaitTimeoutError` (message brut souvent chargé d'un sélecteur CSS) en une phrase compréhensible ("La page Vinted n'a pas répondu à temps. Réessaie...") tout en conservant le détail technique d'origine entre parenthèses — jamais supprimé, seulement relégué en second plan. Point de vérité unique réutilisé par `vinted-publish.ts` et `vinted-edit.ts`, qui divergeaient jusqu'ici (l'un taisait le détail, l'autre l'affichait brut en tête, suite à la demande explicite du 2026-07-16 de voir précisément l'étape en échec).
...
### 6.1bis Import intelligent d'une annonce Vinted existante (sprint extension V1, implémentée le 2026-07-13)

Fait entrer dans ResellOS une annonce Vinted qui existe déjà — le sens inverse de §6.1 (créer sur Vinted à partir de ResellOS). Ne passe **pas** par le Action Engine (`RUN_ACTION`) : c'est une lecture (Vinted → ResellOS), même famille que `ACCOUNT_DETECTED`/`LISTINGS_DETECTED` (§5), pas une écriture sur Vinted qui nécessiterait le pipe checks/preview/confirmation du Action Engine. La seule validation utilisateur requise est le clic explicite sur le bouton injecté — **jamais** déclenché automatiquement à l'ouverture de la page (contrairement à `vinted-profile.ts`, dont la lecture est passive).

**Source des données, hybride** (recherche live effectuée avant implémentation, `extension/src/content/itemSelectors.ts`) :
- Un bloc `<script type="application/ld+json">` de type schema.org `Product`, présent sur chaque page `/items/{id}`, structuré et fiable : titre, description, prix, marque, catégorie, couleur.
- Complété par une lecture DOM ciblée pour ce que le ld+json n'expose pas : galerie photo complète (`img[data-testid^="item-photo-"][data-testid$="--img"]`, dédoublonnée par URL — chaque vignette apparaît plusieurs fois dans le DOM, versions mobile/desktop), taille et état exact (`[itemprop="size"]`/`[itemprop="status"]`, premier nœud texte avant le bouton "infos" imbriqué), matière (sélecteur construit par symétrie, non confirmé en direct — aucun des deux articles testés n'en affichait).

**Résolution du compte** : le pseudo du compte connecté (`LOGGED_IN_USERNAME_SELECTOR`, même sélecteur que §6.1) est utilisé pour **retrouver** un `vinted_accounts` déjà connu — jamais pour en créer un nouveau. La page item ne permet pas de lire le `vinted_user_id` numérique du compte connecté (contrairement à `/member/{id}`, source de vérité pour la création de compte via `vinted-profile.ts`) ; fabriquer un identifiant à partir du seul pseudo créerait une seconde ligne désynchronisée de celle déjà créée par la synchro de profil. Si aucun compte de ce pseudo n'est encore connu, l'import échoue avec un message explicite invitant à visiter d'abord son profil Vinted (`recordSingleItemImport`, `extension/src/background/sync.ts`).

**Dédoublonnage et mise à jour** : upsert sur l'index unique déjà en place `(vinted_account_id, vinted_item_id)`. Contrairement à la synchro passive en masse (§5, qui protège `title`/`brand`/`size`/`image_urls`/`purchase_price` d'un écrasement silencieux), un import explicite déclenché par un clic **rafraîchit tous les champs issus de Vinted** — c'est une action délibérée de l'utilisateur sur cet article précis, pas une synchro de fond. `purchase_price` et `sku` (une fois attribué) restent des concepts ResellOS jamais touchés. `vinted_status` est laissé `null` à l'import (aucune donnée fiable équivalente à `wardrobeApi.ts` sur cette page) — rempli par le prochain passage sur le profil du compte.

```
Content script (vinted-item.ts, injecté sur /items/* sauf /items/new* et /items/*/edit*) :
  1. attend le bloc ld+json → extrait titre/description/prix/marque/catégorie/couleur
  2. lit le pseudo du compte connecté → si absent, n'injecte aucun bouton (rien à importer)
  3. injecte un bouton flottant "Importer dans ResellOS"
  4. au clic : extrait taille/état/matière/galerie complète, envoie IMPORT_ITEM_REQUESTED
Background (sync.ts::recordSingleItemImport) :
  → retrouve le vinted_accounts par pseudo (échec explicite si inconnu)
  → upsert sur (vinted_account_id, vinted_item_id), reprend le SKU si le titre
    Vinted se termine par " #N" (voir extractSkuFromTitle, §5)
  → répond ImportItemResponse { ok, created } au content script, affiché sur place
```
...
### 6.2bis Édition d'une annonce côté app, avec push réel vers Vinted (`edit_listing`, sprint extension V1, implémentée le 2026-07-13)

`EditListingModal.tsx` (`src/components/stock/`) permet de modifier une annonce **déjà persistée** (`Listing`, avec `id`) directement depuis `StockPage.tsx` — jusqu'ici seul `GeneratorPage.tsx`/`EditStep.tsx` permettait d'éditer un `GeneratedListing` *avant* le premier `insert`, aucun chemin `update` n'existait. Toujours une édition **en place côté ResellOS** (`UPDATE ... WHERE id = ...`), pas de table de versions.

**L'UPDATE ne touche que les champs propriétaire de l'app**, en miroir strict de la règle de propriété décrite en §5 (`title`, `description`, `brand`, `category`, `color`, `size`, `material`, `condition`, `price`, `quick_price`, `premium_price`, `keywords`, `vinted_filters`, `image_urls`, plus `last_edited_at`) — **jamais** `vinted_status`/`favourites`/`views`/`synced_at`/`vinted_url`/`vinted_account_id`/`vinted_item_id`, qui restent l'exclusivité de `recordListings()` (`sync.ts`), ni `sku` (§5, jamais réécrit après attribution).
...
**Push réel vers Vinted (`edit_listing`, ActionKind déjà réservé, désormais un vrai `ActionDefinition`)** — pour une annonce déjà liée à Vinted, le bouton "Enregistrer et mettre à jour sur Vinted" remplace l'ancien bandeau d'avertissement passif ("pas de republication automatique") par une vraie action, symétrique de `publish_listing` (§6.1) : mêmes `checks` (authentifié, extension connectée, compte sélectionné) plus `checkListingOwnership` (le compte sélectionné doit être exactement celui auquel l'annonce appartient déjà — pas "un compte au hasard" comme pour une nouvelle publication) et `checkListingAlreadyPublished` (inverse de `checkListingNotAlreadyPublished`). Passe par le même pipe `RUN_ACTION`/Action Engine que §6.1, réutilise `PublishProgressModal`/`ActionStepTimeline` (déjà génériques) pour la progression.

**Limite V1 explicitement validée avec l'utilisateur : texte/attributs uniquement, aucune photo.** Le payload (`EditListingPayload`, `extension/src/lib/messages.ts`) reprend les mêmes champs que `PublishListingPayload` moins `imageUrls`/`packageSize` (déjà définis sur Vinted, pas retouchés). Si les photos ont changé côté ResellOS, l'utilisateur les remplace lui-même directement sur Vinted — la modale le dit explicitement plutôt que de le découvrir après coup.

**URL confirmée en direct, contenu du formulaire non vérifié** : `https://www.vinted.fr/items/{id}/edit` est une route réelle (redirection avec `ref_url` préservé pour un visiteur non authentifié, testé le 2026-07-13 — pas une simple supposition). Le **contenu** du formulaire d'édition n'a en revanche pas pu être vérifié en direct (nécessite d'être authentifié comme propriétaire de l'annonce, jamais fait — aucune manipulation de mot de passe utilisateur). `extension/src/content/vinted-edit.ts` réutilise par hypothèse les sélecteurs de `publishSelectors.ts` (même composant Vinted supposé partagé entre création et édition) via un nouveau module partagé `extension/src/content/formFill.ts` (extrait de `vinted-publish.ts` : `fillTextFields`, `resolveCategory`, `selectMatchingOption`, `verifyLoggedInAccount`, `PublishError` — la logique de remplissage ne doit vivre qu'à un seul endroit maintenant que deux formulaires la partagent). **À reconfirmer lors du premier test live réel** (§119/120 pour `publish_listing` avaient le même statut avant leur validation).

**Bug réel confirmé en test live le 2026-07-24, corrigé le jour même — "Enregistrer les modifications" pouvait faire diverger silencieusement ResellOS de Vinted.** `EditListingModal.tsx::save('none')` écrivait jusqu'ici les champs poussés vers Vinted (titre/description/prix/marque/catégorie/taille/état/couleur/matière) en base pour **n'importe quelle annonce**, y compris une déjà liée à Vinted — sans jamais positionner `vinted_sync_status='sync_pending'`. Conséquence en cascade : la prochaine synchro passive (`extension/src/background/sync.ts::recordListings`, qui rafraîchit `price` sauf si ce statut est `sync_pending`/`sync_failed`) écrasait silencieusement la valeur locale avec celle de Vinted dès qu'une page Vinted était rechargée — un succès annoncé (écriture réelle, confirmée) mais jamais durable. Cause racine directe, distincte de la garde sandbox : le prix passait de 91€ à 90€ dans ResellOS, restait à 91€ sur Vinted (jamais poussé), puis revenait à 91€ dans ResellOS après la synchro suivante. **Corrigé** : pour une annonce `isLinkedToVinted`, `save('none')` n'écrit désormais que les champs strictement internes à ResellOS (`quick_price`/`premium_price`/`keywords`/`vinted_filters`/`image_urls`) — exactement le même traitement que `save('update')` — décision produit validée explicitement (l'alternative, une divergence locale assumée avec un statut dédié, a été écartée). `save()` vérifie aussi désormais le nombre de lignes réellement affectées par l'`UPDATE` (`.select('id')`) avant d'annoncer un succès — un `.update()` Supabase sans `.select()` ne remonte aucune erreur si 0 ligne correspond (RLS, identifiant périmé...). Un message explicite apparaît maintenant dans la modale quand `isLinkedToVinted` est vrai mais `canUpdateOnVinted` est faux (mauvais compte sélectionné dans le filtre en haut de `StockPage.tsx`) — c'est la cause racine du bug initial : le bouton "Enregistrer et mettre à jour sur Vinted" était simplement absent du DOM, sans aucune explication, et l'utilisateur cliquait sur "Enregistrer les modifications" en pensant pousser la mise à jour vers Vinted.

**Garde sandbox retirée le 2026-07-24 (audit du parcours Stock + Publication Vinted, décision explicite de l'utilisateur).** Depuis le 2026-07-15, `checkEditSandboxOnly` (`src/lib/actions/checks.ts`) et sa duplication en défense en profondeur côté extension (`extension/src/background/handlers/editListing.ts`) limitaient tout push `edit_listing` à une unique annonce de test désignée (`vinted_item_id` `9400476768`, "Planche en bois", compte alexisdzk) — objectif : ne jamais risquer de toucher une vraie annonce pendant les tests répétés du pipeline. Les deux gardes sont désormais supprimées (fonction, constante, import, usage dans `checks` — rien laissé en place). Le pipeline reste protégé par les checks génériques inchangés : `checkAuthenticated`, `checkExtensionConnected`, `checkAccountSelected` (le compte sélectionné dans le filtre doit correspondre à `ctx.vintedAccountId`), `checkListingLoaded`, `checkListingOwnership` (l'annonce ciblée doit appartenir au compte Vinted sélectionné), `checkListingAlreadyPublished`, plus `verifyLoggedInAccount()` côté extension (compare le nom d'utilisateur réellement connecté dans l'onglet Vinted à celui attendu). Entrée en phase de validation réelle : la première exécution post-retrait de la garde doit néanmoins être faite manuellement sur l'annonce sandbox (choix de l'utilisateur dans l'UI, plus aucune protection code ne l'impose) avant tout usage sur une vraie annonce.

**Validation réelle terminée le 2026-07-25 — décision d'architecture : soumission par clic MANUEL de l'utilisateur, pas d'automatisation silencieuse.** Cycle de diagnostic complet en conditions réelles (modifications de prix successives sur l'annonce sandbox, plusieurs correctifs d'instrumentation intermédiaires — voir historique git `extension/src/content/vinted-edit.ts`/`extension/src/background/handlers/editListing.ts` pour le détail) :
- Un clic synthétique (`element.click()`, puis séquence complète `pointerdown → mousedown → pointerup → mouseup → click`) atteint bien `document` mais ne déclenche jamais la requête de sauvegarde de Vinted, quelle que soit la fidélité de la simulation.
- La vraie requête (`PUT https://www.vinted.fr/api/v2/item_upload/items/{id}`, payload = état complet de l'annonce, pas un delta) n'a été capturée qu'après un clic **réel** de l'utilisateur (`isTrusted:true`), via `chrome.webRequest` (l'interception `window.fetch`/`XMLHttpRequest` côté page, même installée en `document_start`, ne l'a jamais vue — la bibliothèque réseau de Vinted semble opérer via un mécanisme non interceptable depuis la page).
- La réponse de **cette requête précise** (et d'aucune autre observée durant tout le diagnostic) porte l'en-tête `x-datadome: protected` — la route est explicitement protégée par un service anti-bot.
- **Décision explicite de l'utilisateur (2026-07-25)** : ne pas tenter de rejouer cette requête depuis l'extension pour l'instant — le faire reviendrait à essayer de contourner une protection anti-bot, avec un vrai risque pour le compte Vinted (conditions d'utilisation), et le `PUT` complet exigerait de reconstruire tout l'état de l'annonce (dont les IDs de photos déjà uploadées, non stockés côté ResellOS aujourd'hui) plutôt qu'un simple changement de prix.
- **Architecture retenue et validée en conditions réelles** (prix modifié à plusieurs reprises, persistance confirmée après rafraîchissement des deux côtés) : l'onglet d'édition s'ouvre au premier plan (`chrome.tabs.create({ active: true })`, plus `active: false`) et le pipeline attend un clic réel de l'utilisateur sur "Valider" (`WAITING_FOR_MANUAL_CLICK`) avant de poursuivre vers la phase de vérification déjà existante (relecture réelle du champ, §ci-dessus). `edit_listing` n'est donc **pas** une automatisation 100% silencieuse — c'est un flux semi-assisté (ResellOS prépare/pré-remplit, l'utilisateur valide d'un clic sur Vinted).
- Un chantier dédié à l'automatisation silencieuse (replay de l'appel API interne, ou toute autre approche) pourra être rouvert une fois la bêta stabilisée — pas avant.

Au passage, ce diagnostic a aussi révélé et corrigé deux bugs réels dans `extension/src/background/logger.ts` (ring buffer persisté partagé par tout le pipeline, utilisé pour toute la journalisation diagnostique de l'extension) : une course en écriture (read-modify-write non sérialisé sur `chrome.storage.local`, corrigée par une file d'attente) puis, plus profondément, une course **entre contextes JS** (chaque content script a sa propre instance du module, donc sa propre file, indépendante de celle du background) — corrigée en centralisant toute écriture dans le background via un message `RELAY_LOG_ENTRY`, une seule file d'attente pour toute l'extension.
...
- **Fragilité aux changements de DOM Vinted** : déjà un risque connu pour le scan existant (§9), qui s'étend maintenant aux actions d'écriture (`publish_listing`, §6.1, et `edit_listing`, §6.2bis) — une casse silencieuse y est plus grave qu'un scan qui retourne juste moins de résultats. Plusieurs points de `publish_listing` restent non vérifiés en conditions réelles (liste complète des états, comportement exact de l'upload photo, sélecteur du bandeau d'erreur Vinted après soumission) — voir §6.1. **`edit_listing` (§6.2bis)** : le flux prix (préremplissage + clic manuel utilisateur + relecture de vérification) est validé en conditions réelles (2026-07-25, plusieurs modifications de prix confirmées persistées des deux côtés) ; le formulaire d'édition partage bien le composant de `/items/new`, bouton "Valider" confirmé (pas "Enregistrer" ni "Ajouter"). Reste non revalidé après le retrait des gardes/instrumentation de diagnostic : les branches titre/description/attributs (catégorie/marque/taille/état/couleur/matière), qui avaient chacune leur propre historique de bugs avant le pivot clic-manuel — à retester avant de les considérer beta-ready au même titre que le prix.
